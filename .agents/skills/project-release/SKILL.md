...
“检查能否上线”只授权检查。合并、部署、向用户开放/放量、运行验证分别记录；灰度通常已在生产，日常开发完成不自动进入发布。代码回退不能撤销已经执行的外部业务动作。

1. 确认精确候选/产物、未提交差异、环境、变更、证据和既有授权；按[就绪检查](references/readiness.md)评估缺口。业务模块须有针对当前候选与业务口径的明确人工接受；技术 pass、旧 done、演示截图或部署授权都不能替代它。待人工、退回、未知或过期时不得宣布已就绪。
