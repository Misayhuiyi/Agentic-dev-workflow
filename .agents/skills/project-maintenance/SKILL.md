...
4. 低风险局部更新在本 Skill 内实现与回归，不加载 Superpowers。非简单/高风险实现才交 test-driven-development；初步查证后原因仍不明、反复或跨组件异常才交 systematic-debugging。发现 CVE 本身不启动完整安全审查；只有暴露面、鉴权/租户/数据边界相关，或明确要求安全审查时交 project-security。
5. 普通验证在当前任务内核对新鲜证据；复杂迁移或专门验收才交 project-verification，使用/运维事实变化交 maintaining-project-docs。
