...
4. 只读审查不改代码、不执行未获准外部扫描。仅在因果仍不明或需复杂调查时交 systematic-debugging；已有根因证据直接复用。修复获准后返回 project-development，依据实际风险选择 test-driven-development 等方法；安全边界修复不能因改动行数少而按低风险处理。
