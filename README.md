...
- 简单且明确的改动：理解现有实现，直接修改并验证，不强制额外写设计和计划。
...
当前模板版本：`2.0.0`。包含 **10 个项目生命周期 Skill + 14 个 Superpowers 方法 Skill**，没有示例业务代码、验收样例项目或构建历史。
...
目标、验收和边界清楚，且沿用现有模式时，简述做法后实施即可。不强制新建 spec、计划、并行任务或迭代日志。文案和链接修改也不应被扩大成重构。
...
macOS / Linux 使用 `python3 -B` 替换 `py -3 -B`。脚本使用 Python 3.10+ 标准库；本次实测环境为 Windows / Python 3.11.9，其他系统需在自己的项目验证。
...
`AGENTS.md` 只放长期约束、分级与入口；Skill 名称和描述用于发现；正文在使用时读取；引用文件只展开当前步骤需要的部分。通常一个生命周期 Skill 负责范围，再组合必要方法：

| 方法 | 用途 |
| --- | --- |
| `brainstorming`、`writing-plans` | 未决设计与复杂实施计划 |
| `test-driven-development`、`systematic-debugging` | 功能/修复测试与未知故障定位 |
| `verification-before-completion` | 用新鲜证据支持完成声明 |
| `requesting-code-review`、`receiving-code-review` | 独立审查与审查意见处理 |
...
| `writing-skills`、`using-superpowers` | 编写方法能力及方法发现入口 |

通用知识问答不需要项目开发流程。工具自身的系统/开发者指令、全局插件或强制 Skill 加载可能增加上下文；项目文件不能覆盖更高层规则。因此本模板追求减少重复与无关读取，**不承诺固定 token 节省比例**。
...
本次按要求在当前会话宿主内使用原生子任务和真实临时项目测试，不再调用 Codex CLI。独立子任务不继承父聊天历史，但仍共享宿主规则、工具和可发现的 Skill 目录；这不是完全隔离的跨产品认证。
...
测试项目、日志、评分工具和构建历史留在模板之外。模型请求为 GPT-5.6 / high，宿主实际模型版本、随机性和账单 token 未独立核实。普通知识问答样例仍因宿主要求额外读取了通用 Skill，不能声称零加载成本。未执行其他 AI 产品、macOS/Linux 或生产环境的端到端认证；静态适配一致不代表跨工具实测通过。
...
14 个方法 Skill 来自锁定的 [Superpowers 6.3.0](https://github.com/obra/superpowers/tree/b36e0829c6d0140e93cfef2ca599b1b07d4a7797)。版本、许可、上游文件和单独标识的 UI 元数据见 [来源锁](.ai-workflow/SOURCES.lock.json) 与 [第三方声明](.ai-workflow/THIRD_PARTY_NOTICES.md)。本项目不是这些 AI 工具的官方发行包。
