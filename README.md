...
当前模板版本：`2.2.0`。包含 **11 个项目生命周期 Skill + 14 个 Superpowers 方法 Skill，共 25 个 Skills**，提供 27 个受管适配入口；没有示例业务代码、验收样例项目或构建历史。可行性实验和人工业务验收复用 `project-verification`，不新增 Skill。
...
`2.2.0` 的有限验证在当前会话宿主内完成，未使用 Codex CLI：交付脚本在 Windows / Python 3.11.9 下通过 18 项新增验收回归及 32 项既有状态回归，覆盖待人工、退回、陈旧结论、空技术凭据、旧记录兼容和六项视图。开发副本的 12 项适配测试、9 项来源校验通过；交付根的 25 个 Skill 格式、27 个适配入口和 60 个第三方文件校验通过。一个实际有界实验在 Windows / Python 3.13.9 下验证两个含中文及空格文件名的 Markdown 文件 ZIP 往返后路径与字节一致，仅使用 `project-verification`，未加载 Superpowers。另五个只读情景检查区分待验收、明确退回、错字小改、未知 OCR 可行性和成熟日志小改；未将这些情景当作真实业务验收。

本次旧规则的只读行为基线已能识别“技术通过不等于人工接受”，不能声称其判断曾失败；新版本将这一要求接入可重复的流程、持久状态和脚本门槛。脚本新增断言先在旧实现上失败，再随实现转绿。所有人工同意均为测试夹具或情景假设，不代表你已验收这套模板；仍需按实际业务检查效果。

`2.1.0` 的历史有限验证在当时会话宿主内进行，未使用 Codex CLI：相同初始文档情景的前后对照中，新流程先询问是否调研，没有提前联网或进入设计方法；五个只读情景覆盖拒绝调研、普通解释、材料充分的小项目、仅限给定材料的调研和部分采纳的计划说明。另实际查阅两个公开 GitHub 候选及官方资料，在模板外生成带来源和待审核建议的摘要，没有安装或运行候选代码。25 个 Skill 格式检查、开发工作区 12 项适配测试和 9 项来源校验测试通过；交付根 27 个适配入口及 60 个第三方文件校验一致。这些是有限行为样例和静态/脚本检查，不是所有模型、工具或业务项目的成功保证，也不能当作 `2.2.0` 的验证成绩。

上述真实调研样例随后接受了预设的模拟审核：拒绝一项、待定一项、有条件接受一项。实际只更新原调研记录和唯一计划，接受项附任务与验收，排除项没有进入实施清单；未改代码、依赖或 CI。这验证了该样例的审核落盘行为，不代表测试产品已经开发完成。

`2.0.1` 的历史定向验证：在当时会话宿主中，对同一个小功能分别使用修复前后的规则。旧规则读取 TDD、完成验证及方法参考；新规则只读取 `AGENTS.md` 和 `project-development`，实际功能回归 2/2 通过。另一个简单 Bug 修复同样未读取 Superpowers，先复现失败再修复，3/3 通过。五类只读路由情景检查覆盖普通问答、Skill 解释、文档小改、复杂设计和生产诊断；这些是路由检查，不能冒充五个真实业务项目的端到端测试。24 个 Skill 通过 UTF-8 格式检查，开发工作区的适配测试 11/11、来源补丁校验测试 9/9 通过；交付物的 26 个适配入口与 60 个第三方文件校验一致。未测账单 token 或其他宿主自动发现行为。

以下是 `2.0.0` 在当时会话宿主内使用原生子任务和真实临时项目得到的历史有限样例，未调用 Codex CLI。独立子任务不继承父聊天历史，但仍共享宿主规则、工具和可发现的 Skill 目录；这不是完全隔离的跨产品认证，也不能直接证明 `2.0.1` 的触发边界已通过运行验证。
