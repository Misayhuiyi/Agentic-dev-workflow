...
当前模板版本：`2.1.0`。包含 **11 个项目生命周期 Skill + 14 个 Superpowers 方法 Skill，共 25 个 Skills**，提供 27 个受管适配入口；没有示例业务代码、验收样例项目或构建历史。
...
│   │   └── research-brief.md    需要持久保存调研时使用；不自动创建调研目录
...
`2.1.0` 在当前会话宿主内进行了有限验证，未使用 Codex CLI：相同初始文档情景的前后对照中，新流程先询问是否调研，没有提前联网或进入设计方法；五个只读情景覆盖拒绝调研、普通解释、材料充分的小项目、仅限给定材料的调研和部分采纳的计划说明。另实际查阅两个公开 GitHub 候选及官方资料，在模板外生成带来源和待审核建议的摘要，没有安装或运行候选代码。25 个 Skill 格式检查、开发工作区 12 项适配测试和 9 项来源校验测试通过；交付根 27 个适配入口及 60 个第三方文件校验一致。这些是有限行为样例和静态/脚本检查，不是所有模型、工具或业务项目的成功保证。
