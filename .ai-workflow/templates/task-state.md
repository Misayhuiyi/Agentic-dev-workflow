...
有分阶段交付时在此保存紧凑合同：里程碑/任务 ID、适用 spec/plan 版本、批准人及原始依据、范围与不做什么、最大不确定性/验证、可体验产物、验收与时间/步骤/费用预算、依赖、风险/回退、确认人。近期详细、远期可调整；无此需要的小任务不补造完整计划。批准绑定范围与版本；多阶段明确获批仍逐项满足依赖与人工门槛。

用引用而非复制内容串联：业务问题 → 需求/验收 ID → 决策 → 本任务 → 代码候选 → 测试/Eval → 人工验收 → 发布版本 → 业务结果。多项复杂需求才用表；简单任务可一行说明。范围或证据改变时更新受影响合同与 next_step，不添加第二状态块。

本合同是软约束，脚本不会分配获准任务或拦截工具。业务假设失败、验收退回、范围扩大、关键依赖不可用、超预算或新高风险动作时停止受影响后续工作；已有权限内且独立的任务可继续。外部操作、生产凭据与费用限制由实际环境分别控制。
